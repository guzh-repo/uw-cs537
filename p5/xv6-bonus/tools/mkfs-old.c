...
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){
    ...
    assert(fbn < MAXFILE);
    if(fbn < NDIRECT){
      if(xint(din.datab_idx[fbn]) == 0){
        din.datab_idx[fbn] = xint(freeblock++);
        usedblocks++;
      }
      x = xint(din.datab_idx[fbn]);
    } else {
      if(xint(din.indirectb_idx) == 0){
        // printf("allocate indirect block\n");
        din.indirectb_idx = xint(freeblock++);
        usedblocks++;
      }
      // printf("read indirect block\n");
      rsect(xint(din.indirectb_idx), (char*)indirect);
      if(indirect[fbn - NDIRECT] == 0){
        indirect[fbn - NDIRECT] = xint(freeblock++);
        usedblocks++;
        wsect(xint(din.indirectb_idx), (char*)indirect);
      }
