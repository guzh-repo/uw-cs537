...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    if((addr = ip->datab_idx[bn]) == 0)
      ip->datab_idx[bn] = addr = balloc(ip->dev);
    return addr;
  }
  bn -= NDIRECT;
  ...
  if(bn < NINDIRECT){
    ...
    if((addr = ip->indirectb_idx) == 0)
      ip->indirectb_idx = addr = balloc(ip->dev);
...
static uint get_checksum(struct inode *ip, uint bn) {
    if (bn < NDIRECT) return ip->datab_chk[bn];
    if (bn >= NDIRECT + NINDIRECT) panic("checksum_ondisk: bn out of range");
    if (ip->indirectb_idx == 0) panic("checksum_ondisk: missing indirect data block");
    struct buf *bp = bread(ip->dev, ip->indirectb_idx);
    uint *indirect_block = (uint*)bp->data;
    uint chk = indirect_block[NINDIRECT+(bn-NDIRECT)];
    brelse(bp);
    return chk;
}
...
static void set_checksum(struct inode *ip, uint bn, uint checksum) {
    ...
    struct buf *bp = bread(ip->dev, ip->indirectb_idx);
    uint *indirect_block = (uint*)bp->data;
    indirect_block[NINDIRECT+(bn-NDIRECT)] = checksum;
    bwrite(bp);
    brelse(bp);
}
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    //cprintf("block:0x%x checksum:0x%x checksum_disk:0x%x\n", sector_number, chk, checksum_disk);
