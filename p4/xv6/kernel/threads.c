...
int sys_clone(void) {
    int function_addr; argint(0, &function_addr); // WARN: do not run in kernel space
    int data_addr; argint(1, &data_addr);
    char *stack_low_addr;
    if (argptr(2, &stack_low_addr, PGSIZE) != 0) return -1;
