...
//   0..4K            : unmapped page
//   4K..brk          : user memory (text, data, heap)
//   brk..s_top       : unmapped space for heap or stack
//   s_top..640K      : stack
...
// set permission to PTE_W to allow write
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz, int permission)
{
  char *mem;
  uint a;
  ...
  for(; a < newsz; a += PGSIZE){
    ...
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, PADDR(mem), permission|PTE_U);
  }
  return newsz;
}
...
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint brk, uint s_top)
{
  pde_t *d;
  pte_t *pte;
  uint pa, i;
  ...
  for(i = PGSIZE; i < brk; i += PGSIZE){
    ...
    if(mappages(d, (void*)i, PGSIZE, PADDR(mem), (*pte&PTE_W)|PTE_U) < 0)
      goto bad;
  }

  for(i = s_top; i < USERTOP; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
